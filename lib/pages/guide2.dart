...
class GuidePage2 extends StatefulWidget {
  final Place? d;
  GuidePage2({Key? key, required this.d}) : super(key: key);
...
class _GuidePage2State extends State<GuidePage2> {
  ...
  Future addMarker() async {
    List m = [
      Marker(
          ...
          position: LatLng(
              data!['paths2'][0]['latitude'], data!['paths2'][0]['longitude']),
      ...
      Marker(
          ...
          position: LatLng(
              data!['paths2'][data!['paths2'].length - 1]['latitude'],
              data!['paths2'][data!['paths2'].length - 1]['longitude']),
  ...
  Future _getPolyline() async {
    PolylineResult result = await polylinePoints.getRouteBetweenCoordinates(
      ...
      PointLatLng(
          data!['paths2'][0]['latitude'], data!['paths2'][0]['longitude']),
      PointLatLng(data!['paths2'][data!['paths2'].length - 1]['latitude'],
          data!['paths2'][data!['paths2'].length - 1]['longitude']),
  ...
  _addPolyLine() async {
    ...
    for (var data in data!['paths2']) {
      PointLatLng point = PointLatLng(data['latitude'], data['longitude']);
      points.add(point);
    }

    final List<LatLng> polylineCoordinates = [];
    List<List<PointLatLng>> paths2 = await _getPaths2(points);
    for (int i = 0; i < points.length - 1; i++) {
      ...
      if (i < paths2.length) {
        for (int j = 0; j < paths2[i].length; j++) {
          final pathLatitude = paths2[i][j].latitude;
          final pathLongitude = paths2[i][j].longitude;
...
// Add last point
  ...
  Future<List<List<PointLatLng>>> _getPaths2(List<PointLatLng> points) async {
    List<List<PointLatLng>> paths2 = [];
    for (int i = 0; i < points.length - 1; i++) {
      PolylineResult result = await polylinePoints.getRouteBetweenCoordinates(
        Config().mapAPIKey,
        points[i],
  ...
  void _add1(List<dynamic> paths2Data) {
    _geofenceList.clear();
    _geofenceAudioUrls.clear();
    for (int i = 0; i < paths2Data.length; i++) {
      final path2 = paths2Data[i] as Map<String, dynamic>;
      final name = path2['name'];
      final latitude = path2['latitude'];
      final longitude = path2['longitude'];
      final audio = path2['audioUrl'];
      final radius = [
        GeofenceRadius(
          id: path2['radius'],
          length: 25,
  ...
  void _add(List<dynamic> paths2Data) async {
    final ImageCropper imageCropper = ImageCropper();
    for (final pathData in paths2Data) {
  ...
  Future<void> getMealsData() async {
    ...
    if (snapshot.exists) {
      ...
      final paths2Data = data['paths2'] as List<dynamic>;
      _add(paths2Data);
      _add1(paths2Data);
    }
  }
        ...
        Expanded(
          child: data!.isEmpty
              ...
              : ListView.separated(
                  ...
                  itemCount: data!['paths2'].length,
                  itemBuilder: (BuildContext context, int index) {
                    final List paths2 = data!['paths2'];
                    paths2.sort((a, b) => a['name'].compareTo(b['name']));
                    final path2Data = paths2[index];
                    final name = path2Data['name'];
                    final image = path2Data['image'];
                    final detail = path2Data['detail'];
                    final audioUrl = path2Data['audioUrl'];
                    final latitude = path2Data['latitude'] as double;
                    final longitude = path2Data['longitude'] as double;
                    return Padding(
