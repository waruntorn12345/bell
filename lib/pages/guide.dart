...
class _GuidePageState extends State<GuidePage> {
  ...
  Future addMarker() async {
    List m = [
      Marker(
          ...
          position: LatLng(
              data!['paths'][0]['latitude'], data!['paths'][0]['longitude']),
      ...
      Marker(
          ...
          position: LatLng(
              data!['paths'][data!['paths'].length - 1]['latitude'],
              data!['paths'][data!['paths'].length - 1]['longitude']),
  ...
  Future _getPolyline() async {
    PolylineResult result = await polylinePoints.getRouteBetweenCoordinates(
      ...
      PointLatLng(
          data!['paths'][0]['latitude'], data!['paths'][0]['longitude']),
      PointLatLng(data!['paths'][data!['paths'].length - 1]['latitude'],
          data!['paths'][data!['paths'].length - 1]['longitude']),
  ...
  _addPolyLine() async {
    ...
    for (var data in data!['paths']) {
      // Check if the name of the path contains a dot "."
    ...
    List<List<PointLatLng>> paths = await _getPaths(points);
    for (int i = 0; i < points.length - 1; i++) {
      ...
      if (i < paths.length) {
        for (int j = 0; j < paths[i].length; j++) {
          final pathLatitude = paths[i][j].latitude;
          final pathLongitude = paths[i][j].longitude;
  ...
  Future<List<List<PointLatLng>>> _getPaths(List<PointLatLng> points) async {
    List<List<PointLatLng>> paths = [];
    for (int i = 0; i < points.length - 1; i++) {
      PolylineResult result = await polylinePoints.getRouteBetweenCoordinates(
        Config().mapAPIKey,
        points[i],
  ...
  void animateCamera() {
    mapController.animateCamera(CameraUpdate.newCameraPosition(CameraPosition(
      target:
          LatLng(data!['paths'][0]['latitude'], data!['paths'][0]['longitude']),
      zoom: 15,
      tilt: 50.0,
      bearing: 45.0,
    )));
  ...
  void _add1(List<dynamic> pathsData) {
    _geofenceList.clear();
    _geofenceAudioUrls.clear();
    for (int i = 0; i < pathsData.length; i++) {
      final path = pathsData[i] as Map<String, dynamic>;
      final name = path['name'];
      if (!name.contains('.')) {
        // check if the name contains a dot
        final latitude = path['latitude'];
        final longitude = path['longitude'];
        final audio = path['audioUrl'];
        final radius = [
          GeofenceRadius(
            id: path['radius'],
            length: 25,
  ...
  void navigateToNextLocation(String location) {
    // Check if the location parameter matches any of the next locations
    if (location == 'restaurant1') {
      // Navigate to restaurant1 location
      print('Navigating to restaurant1 location');

      // Get the latitude and longitude of restaurant1 from data
      double latitude = data!['paths'][7]['latitude'];
      double longitude = data!['paths'][7]['longitude'];

      // Launch Google Maps with the location coordinates
      launch('https://www.google.com/maps?q=$latitude,$longitude');
    } else if (location == 'restaurant2') {
      // Navigate to restaurant2 location
      print('Navigating to restaurant2 location');
      double latitude = data!['paths'][8]['latitude'];
      double longitude = data!['paths'][8]['longitude'];

      // Launch Google Maps with the location coordinates
      launch('https://www.google.com/maps?q=$latitude,$longitude');
    } else if (location == 'restaurant3') {
      // Navigate to restaurant3 location
      print('Navigating to restaurant3 location');
      double latitude = data!['paths'][9]['latitude'];
      double longitude = data!['paths'][9]['longitude'];
  ...
  void _add(List<dynamic> pathsData) async {
    final ImageCropper imageCropper = ImageCropper();
    final RegExp noDotRegex =
        RegExp(r"^[^.]+$"); // สร้าง regex เพื่อค้นหา string ที่ไม่มีจุด
    for (final pathData in pathsData) {
      final name = pathData['name'] as String;
      if (noDotRegex.hasMatch(name)) {
        // ตรวจสอบว่า name ไม่มีจุด
        final detail = pathData['detail'] as String;
        final image = pathData['image'] as String;
        final latitude = pathData['latitude'] as double;
        final longitude = pathData['longitude'] as double;
  ...
  Future<void> getMealsData() async {
    ...
    if (snapshot.exists) {
      ...
      final pathsData = data['paths'] as List<dynamic>;
      _add(pathsData);
      _add1(pathsData);
    }
  }
                    ...
                    children: <Widget>[
                      ...
                      Row(
                        ...
                        children: <Widget>[
                          ElevatedButton(
                            onPressed: () =>
                                navigateToNextLocation('restaurant1'),
                            child: Text(
                              'ร้านอาหาร 1',
                              style: TextStyle(
                                fontSize: 15.0,
                          ...
                          ElevatedButton(
                            onPressed: () =>
                                navigateToNextLocation('restaurant2'),
                            child: Text(
                              'ร้านอาหาร 2',
                              style: TextStyle(
                                fontSize: 15.0,
                          ...
                          ElevatedButton(
                            onPressed: () =>
                                navigateToNextLocation('restaurant3'),
                            child: Text(
                              'ร้านอาหาร 3',
                              style: TextStyle(
                                fontSize: 15.0,
          ...
          child: data!.isEmpty
              ...
              : ListView.separated(
                  ...
                  itemBuilder: (BuildContext context, int index) {
                    final List paths = data!['paths'];
                    paths.sort((a, b) => a['name'].compareTo(b['name']));
                    final pathData = paths[index];
                    final name = pathData['name'];
                    final hasDot = name.contains('.');
                    final image = pathData['image'];
                    final detail = pathData['detail'];
                    final audioUrl = pathData['audioUrl'];
                    final latitude = pathData['latitude'] as double;
                    final longitude = pathData['longitude'] as double;
                    if (hasDot) {
                      return SizedBox();
